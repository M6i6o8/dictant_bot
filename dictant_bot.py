...
def main():
    """Главная функция"""
    print("\n" + "="*60)
    print("🚀 ЗАПУСК БОТА (12:30 ЗАДАНИЕ - 12:40 ОТВЕТ)")
    print("="*60)
    ...
    print(f"🕐 Текущее время UTC: {current_hour}:{current_minute}")
    print(f"🕐 Текущее время МСК: {current_hour+3}:{current_minute}")
    
    # Проверяем время отправки (9:30 UTC = 12:30 МСК, 9:40 UTC = 12:40 МСК)
    is_task_time = (current_hour == 9 and 30 <= current_minute < 40)
    is_answer_time = (current_hour == 9 and 40 <= current_minute < 50)
    
    if not (is_task_time or is_answer_time):
        print("⏰ Не время для отправки. Бот работает в 12:30 и 12:40 МСК")
        print("   Для теста закомментируй эту проверку в коде")
        return  # Раскомментируй для продакшена
    ...
    print(f"   📖 Разбор: {sentence.get('explanation', 'Есть в вечернем посте')[:100]}...")
    
    # ===== ФОРМИРУЕМ СООБЩЕНИЕ ПО ВРЕМЕНИ =====
    message = ""  # Инициализируем переменную
    
    if is_task_time:  # 12:30 МСК - только задание
        ...
        message += f"⏳ <b>Правильный перевод и разбор грамматики придут сегодня в 12:40</b>"
        
        print("\n📨 Отправляем ЗАДАНИЕ на 12:30...")
        
    elif is_answer_time:  # 12:40 МСК - перевод + разбор
        ...
        print("\n📨 Отправляем ОТВЕТ на 12:40...")
    
    else:  # Любое другое время - тестовый режим
        message = f"📝 <b>ТЕСТОВЫЙ ДИКТАНТ</b>\n\n"
        message += f"<b>Тема:</b> {sentence['topic']}\n"
        message += f"<b>Сложность:</b> {sentence.get('difficulty', 'легко')}\n\n"
        message += f"🇬🇧 <b>Предложение:</b>\n"
        message += f"<i>{sentence['en']}</i>\n\n"
        message += f"🇷🇺 <b>Перевод:</b>\n"
        message += f"<i>{sentence['ru']}</i>\n\n"
        message += f"📊 <b>Разбор:</b>\n"
        message += f"{sentence.get('explanation', 'Разбор будет в 12:40')}\n\n"
        message += f"⏳ <b>ТЕСТОВЫЙ ЗАПУСК (не по расписанию)</b>"
        
        print("\n📨 Отправляем ТЕСТОВОЕ сообщение...")
    
    # Проверяем, что сообщение создано
    if not message:
        print("❌ КРИТИЧЕСКАЯ ОШИБКА: сообщение не создано")
        return
